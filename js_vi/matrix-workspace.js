...
(function () {
  ...
  function renderNextChunk() {
    ...
    for (let i = start; i < end; i++) {
      ...
      on(input, 'input', () => { item.userAnswer = input.value; App.saveAnswer(item.rawIndex, input.value); checkOne(tr, item); App.saveState(); });
  ...
  function handleInputKeydown(event, viewIndex) {
    if (event.key === 'Enter' || event.key === 'ArrowDown') { event.preventDefault(); focusInput(viewIndex + 1); }
    else if (event.key === 'ArrowUp') { event.preventDefault(); focusInput(viewIndex - 1); }
    else if (event.key === 'Shift' || event.key === 'ArrowRight') { const item = App.state.vocabList[viewIndex]; if (item) speakByMode(item); }
  }
